...
import { 
  ...
  renderPtStatus } from '../methods/User';
...
const selGcpContactMU = computed(() => JSON.parse(JSON.stringify(store.state.selectlist.gcpContactList)));
...
getGcpByIdOnDone(result=>{
  ...
  nowGcpContactDOM.value.setValue(parseInt(nowGcpContactId.value));

  if(getData.gcp_contact){
    nowGcpContactName.value = getData.gcp_contact.name;
...
const data_hist = ref([]);
const columns_hist = [
  {title:"狀態", data:"status", defaultContent: "-", render: renderPtStatus},
  ...
  {title:"清查日", data:"date", className: "dt-center", defaultContent: "-", render: toTWDate},
                        ...
                        <MDBTabContent :key="updateKey" style="height: calc(100% - 6rem);">
                          ...
                          <MDBTabPane tabId="contact" class="h-100">
                            <MDBRow class="overflow-auto align-content-start h-100">
                              ...
                              <MDBCol col="12" class="mt-2">
                                <MDBTextarea :disabled="!nowGcpNeedContact" size="sm" label="備註" rows="2" v-model="nowGcpContactCom" />
                              </MDBCol>
                            </MDBRow>
                          </MDBTabPane>
                        </MDBTabContent>
